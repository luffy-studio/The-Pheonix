...
const LoginSignupForm: React.FC = () => {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-gray-200 to-indigo-200">
      <div className="relative w-[850px] h-[550px] bg-[#7494ec] rounded-[30px] shadow-2xl overflow-hidden">
        {/* Blue Background Overlay */}
        <div
          className={`absolute top-0 left-0 w-full h-full bg-[#ffffff] rounded-[150px] transition-all duration-[1800ms] ease-in-out z-0 ${isActive ? "-translate-x-1/2" : "translate-x-1/2"
          }`}
        >
          {/* Register Form */}
          <div className={`absolute right-0 w-1/2 h-full flex items-center text-center p-10 z-10 transition-all duration-700 ease-in-out ${isActive ? "opacity-100 pointer-events-none" :"opacity-0 pointer-events-none" }`}>
           ...
           {/* Login Form */}
        <div className={`absolute w-1/2 h-full flex items-center text-center p-10 z-10 transition-all duration-700 ease-in-out ${isActive ? "translate-x-[-200%] opacity-0" : "right-0 opacity-100"}`}>
          <form className="w-full">
